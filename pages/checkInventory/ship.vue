<template>
	<view class="content">
		<view class="example">
			...
			<button type="primary" v-bind:disabled="currentSteps != 1" v-on:click="scanWarehouse"><text>扫库位码</text></button>
					...
					<view class="uni-card__content uni-card__content--pd">
						<view v-for="item in material.goods" v-bind:key="item" class="wxc-list">
							<view class="wxc-list-title-text">
								{{ material.storage == null ? '正在等待库位码，可继续物料' : '发货货架' }}
			...
			<button type="primary" v-bind:disabled="!sureOutlibrarys" @click="sureOutlibrary">确认备件发货</button>
...
import outlibraryModel from '@/model/checkInventoryShipModel.js';
