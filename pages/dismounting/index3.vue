<template>
	<view class="content">
		<view class="example">
			...
			<button type="primary" v-bind:disabled="currentSteps > 1" v-on:click="scanMaterial"><text>扫出库码</text></button>
			...
			<!-- <view v-show="sureOutlibrarys" class="uni-textarea">
                <textarea placeholder-style="color:#F76260" v-model="material.reason" placeholder="请填写组装出库的原因"/>
            </view> -->
			<button type="primary" v-bind:disabled="!sureOutlibrarys" @click="sureOutlibrary">确认组装出库</button>
...
import { checkLocal, saveAssemOutInfo } from '@/api/dismounting.js';
import dismountingModel from '@/model/dismountingModel.js';
import { mapState } from 'vuex';
export default {
	data() {
		return {
			material: dismountingModel,
	...
	methods: {
		...
		//确定组装出库
		sureOutlibrary: function() {
			var _this = this;
			saveAssemOutInfo(this.material.generateModel()).then(data => {
				var [error, res] = data;
